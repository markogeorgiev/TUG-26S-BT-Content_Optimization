"""
Per-query Kendall's tau-b between each content feature and document rank.

For every query and every retrieval model, this computes Kendall's tau-b between
...
NEGATIVE tau means "higher feature value -> better (smaller) rank number", i.e.
the feature is associated with ranking *higher*. A POSITIVE tau means the feature
is associated with ranking *lower*. (This is inherent to correlating against rank;
nothing is flipped for you.)

tau-b (not tau-a) is used because it corrects for ties, which are common in
content features (e.g. integer counts). pandas' ``corr(method="kendall")``
computes tau-b.

Inputs
------
  * features : output/content_features.csv   (doc_id + feature columns)
  * rankings : rankings/full_retriever/rankings_{bm25,tfidf,sbert,e5,
               colbert,cross_encoder}.csv
               columns: query_id, doc_id, doc_rank, doc_score, model

Output
------
  output/per_query_kendall/kendall_{bm25,tfidf,sbert,e5,colbert,cross_encoder}.csv
  Wide table: one row per query_id, one column per feature, cell = Kendall tau-b.

Usage
-----
    python.exe per_query_kendall_tau_b.py
    python.exe per_query_kendall_tau_b.py --features output/content_features.csv \
        --rankings-dir rankings/full_retriever --out-dir output/per_query_kendall
"""
...
def per_query_kendall(
    ranking: pd.DataFrame,
    features: pd.DataFrame,
    feature_cols: List[str],
) -> pd.DataFrame:
    """Wide table: index = query_id, columns = features, cell = Kendall tau-b vs rank."""
    merged = ranking.merge(features, on="doc_id", how="inner")

    rows: Dict[str, Dict[str, float]] = {}
    for query_id, group in merged.groupby("query_id", sort=True):
        # Kendall tau-b of every feature against doc_rank in one shot, then keep
        # the doc_rank column (its correlation with each feature).
        corr = group[feature_cols + ["doc_rank"]].corr(method="kendall")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="output directory (default: output/per_query_kendall)",
    )
    ...
    for label, filename in MODEL_FILES.items():
        ...
        print(f"[{label}] {n_queries} queries; computing per-query Kendall tau-b")
        result = per_query_kendall(ranking, features, feature_cols)

        out_path = args.out_dir / f"kendall_{label}.csv"
