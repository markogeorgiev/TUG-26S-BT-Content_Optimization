"""
Significance testing of per-query Kendall tau-b correlations, per retrieval model.

For each model we already have, per query, the Kendall tau-b correlation between
every content feature and document rank (see per_query_kendall_tau_b.py). With 30
queries that gives, per feature, a sample of 30 correlation coefficients. This
script asks, for each feature: is that correlation reliably non-zero across
queries?
...
Per model, per feature:
  ...
  3. Otherwise:
       ...
       mean_tau  = mean of coefficients          (the EFFECT SIZE)
       std_tau   = sample std, ddof=1
       se        = std_tau / sqrt(n)
...
Inputs  : rank_relationship_analysis/per_query_kendall/kendall_<model>.csv
          (wide: query_id column + one column per feature)
Outputs : rank_relationship_analysis/significance_kt/significance_<model>.csv
          rank_relationship_analysis/significance_kt/significance_all_models.csv

Usage
-----
    python significance_kt.py
    python significance_kt.py --in-dir <dir> --out-dir <dir> --alpha 0.05
"""
...
# Output label -> per-query Kendall filename. (file token, display name)
MODELS: Dict[str, Tuple[str, str]] = {
    "bm25": ("BM25", "kendall_bm25.csv"),
    "tfidf": ("TF-IDF", "kendall_tfidf.csv"),
    "sbert": ("SBERT", "kendall_sbert.csv"),
    "e5": ("E5", "kendall_e5.csv"),
    "colbert": ("ColBERT", "kendall_colbert.csv"),
    "cross_encoder": ("Cross-Encoder", "kendall_cross_encoder.csv"),
}

# Columns that are not features in the per-query Kendall files.
NON_FEATURE_COLUMNS = {"query_id"}

# Output column order (exact).
OUTPUT_COLUMNS = [
    "model", "feature", "n_queries", "mean_tau", "std_tau", "se",
    "t", "p_value", "p_corrected", "significant",
]
FLOAT_COLUMNS = ["mean_tau", "std_tau", "se", "t", "p_value", "p_corrected"]
...
def test_model(
    ...
    feature_cols = [c for c in kendall.columns if c not in NON_FEATURE_COLUMNS]

    records: List[dict] = []
    for feature in feature_cols:
        values = kendall[feature].dropna().to_numpy(dtype=float)
        n = values.size

        if n < MIN_VALID:
            records.append({
                "model": model_name, "feature": feature, "n_queries": n,
                "mean_tau": np.nan, "std_tau": np.nan, "se": np.nan,
                "t": np.nan, "p_value": np.nan,
            })
            continue

        mean_tau = float(np.mean(values))
        std_tau = float(np.std(values, ddof=1))
        se = std_tau / np.sqrt(n)
        t_stat, p_value = stats.ttest_1samp(values, popmean=0.0)

        records.append({
            "model": model_name, "feature": feature, "n_queries": n,
            "mean_tau": mean_tau, "std_tau": std_tau, "se": se,
...
def main() -> None:
    ...
    parser.add_argument(
        "--in-dir", type=Path,
        default=ANALYSIS_DIR / "per_query_kendall",
        help="directory with kendall_<model>.csv files",
    )
    parser.add_argument(
        "--out-dir", type=Path,
        default=ANALYSIS_DIR / "significance_kt",
    ...
    for token, (model_name, filename) in MODELS.items():
        ...
        kendall = pd.read_csv(path)
        result = test_model(kendall, model_name, alpha=args.alpha)
